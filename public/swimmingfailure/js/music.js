JSON_FILEPATH = "/swimmingfailure/media/music/!music.json";
...
function makeTrackElems(json) {
    ...
    for (let i in originals) {
        let musicpath = MUSIC_FOLDERPATH + originals[i].title.toLowerCase().replace(/\s/g, '').replace(/'/g, '');
        let artpath = ART_FOLDERPATH + originals[i].title.toLowerCase().replace(/\s/g, '').replace(/'/g, '');
        let html = 
            `<div class="track" data-track="${musicpath}.mp3" style="background-image: url(${artpath}.jpg)">
    ...
    for (let i in covers) {
        let musicpath = MUSIC_FOLDERPATH + covers[i].title.toLowerCase().replace(/\s/g, '').replace(/'/g, '');
        let artpath = ART_FOLDERPATH + covers[i].title.toLowerCase().replace(/\s/g, '').replace(/'/g, '');
        let html = 
            `<div class="track" data-track="${musicpath}.mp3" style="background-image: url(${artpath}.jpg)">
